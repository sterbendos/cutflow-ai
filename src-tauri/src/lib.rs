...
// -------------------------------------------------------------
// Tauri Command: export_video
// Uses FFmpeg sidecar to export keep segments to an MP4 file on disk.
// This re-encodes each kept segment then concatenates them to ensure
// a compatible final MP4 output.
// -------------------------------------------------------------

#[tauri::command]
async fn export_video(app: AppHandle, output_path: String) -> Result<String, String> {
    let (source_video_path, keep_segments) = {
        let state_read = TIMELINE_STATE
            .read()
            .map_err(|e| format!("Lock poisoned: {e}"))?;

        let path = state_read.source_video_path.clone();
        let keeps: Vec<EdlSegment> = state_read
            .edl
            .iter()
    ...
    let tmp_dir = std::env::temp_dir().join(format!("cutflow_export_{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&tmp_dir).map_err(|e| format!("Failed to create temp dir: {e}"))?;

    let mut seg_files: Vec<String> = Vec::new();

    for (i, seg) in keep_segments.iter().enumerate() {
        let start = format!("{}", seg.start);
        let end = format!("{}", seg.end);
        let seg_out = tmp_dir.join(format!("seg_{}.mp4", i));
        ...
        let out = app
            ...
            .output()
            .await
            .map_err(|e| format!("Failed to run ffmpeg for segment: {e}"))?;

        if !out.status.success() {
            let stderr = String::from_utf8_lossy(&out.stderr);
            return Err(format!("FFmpeg failed to encode segment {}: {stderr}", i));
        }

        seg_files.push(seg_out_str);
    }
    ...
    std::fs::write(&concat_list_path, concat_contents).map_err(|e| format!("Failed to write concat file: {e}"))?;
    ...
    if !join_out.status.success() {
        let stderr = String::from_utf8_lossy(&join_out.stderr);
        return Err(format!("FFmpeg failed to concat segments: {stderr}"));
    }
